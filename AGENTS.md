...
- Changelog: Every new feature, update, or milestone must be added to the timeline in `src/data/changelog.ts`.
...
5. Record update/milestone in `src/data/changelog.ts` when adding features or significant updates.
...
- [ ] Changelog updated in `src/data/changelog.ts` if new feature or update introduced
